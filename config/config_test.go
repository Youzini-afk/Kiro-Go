...
func TestSetApiKeyUpdatesMultiKeyEntryForEnvOverride(t *testing.T) {
	if err := Init(filepath.Join(t.TempDir(), "config.json")); err != nil {
		t.Fatalf("init config: %v", err)
	}
	if _, err := AddApiKey(ApiKeyEntry{Name: "existing", Key: "sk-existing", Enabled: true}); err != nil {
		t.Fatalf("add existing key: %v", err)
	}

	SetApiKey("sk-env", true)

	if got := GetApiKey(); got != "sk-env" {
		t.Fatalf("expected legacy api key to be updated, got %q", got)
	}
	if !IsApiKeyRequired() {
		t.Fatalf("expected API key requirement to be enabled")
	}
	if found := FindApiKeyByValue("sk-env"); found == nil || !found.Enabled {
		t.Fatalf("expected env API key to authenticate via multi-key list, got %#v", found)
	}
}

func TestSetApiKeyReplacesMigratedEntry(t *testing.T) {
	if err := Init(filepath.Join(t.TempDir(), "config.json")); err != nil {
		t.Fatalf("init config: %v", err)
	}
	created, err := AddApiKey(ApiKeyEntry{Name: "legacy", Key: "sk-old", Enabled: true, Migrated: true})
	if err != nil {
		t.Fatalf("add migrated key: %v", err)
	}

	SetApiKey("sk-new", true)

	if found := FindApiKeyByValue("sk-new"); found == nil || found.ID != created.ID {
		t.Fatalf("expected migrated entry to be replaced in-place, got %#v", found)
	}
	if old := FindApiKeyByValue("sk-old"); old != nil {
		t.Fatalf("expected old migrated key to be replaced, got %#v", old)
	}
}
