...
func TestGetNextReturnsExpiredTokenForRequestPathRefresh(t *testing.T) {
	p := &AccountPool{}
	account := config.Account{
		ID:          "acct-expired",
	...
	got := p.GetNext()
	if got == nil {
		t.Fatalf("expected expired token account to remain selectable for request-path refresh")
	}
	if got.ID != account.ID {
		t.Fatalf("expected account %q, got %q", account.ID, got.ID)
	}
...
func TestGetNextForModelReturnsExpiredTokenForRequestPathRefresh(t *testing.T) {
	p := &AccountPool{
		accounts: []config.Account{{
			ID:        "expired-model-account",
			Enabled:   true,
			ExpiresAt: time.Now().Unix() - 60,
		}},
		cooldowns:   make(map[string]time.Time),
	...
	p.SetModelList("expired-model-account", []string{"claude-sonnet-4.5"})

	got := p.GetNextForModelExcluding("claude-sonnet-4.5", nil)
	if got == nil {
		t.Fatalf("expected expired token account to remain selectable for request-path refresh")
	}
	if got.ID != "expired-model-account" {
		t.Fatalf("expected expired-model-account, got %#v", got)
	}
}
